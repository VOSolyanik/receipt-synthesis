...
| `--split` | *required* | Fraction of **personas** assigned to train. Pass `0.5` unless your consumer really does train: **nothing is trained on this dataset**, and the measurement side has to be large enough to carry a per-class figure for the thinnest class. The partition is by persona because annual limits are cumulative per persona; it is **not** stratified. |
